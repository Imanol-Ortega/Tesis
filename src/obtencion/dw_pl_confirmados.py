...
CSV_DIR = os.path.join(BASE_DIR, 'data', 'mast', 'csv')

# Ajusta esto según tu PC
MAX_WORKERS = 1
...
def descargar_curvas(test_mode=True, limit=5):
    print(f"\n>>> [ADQUISICIÓN DE DATOS PARALELA] Workers: {MAX_WORKERS}")
    ...
    try:
        ...
        if test_mode:
            df = df.head(limit)
            print(f"--- MODO PRUEBA: {limit} planetas ---")
        else:
            print(f"--- MODO COMPLETO: {len(df)} planetas ---")
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        futures = {executor.submit(procesar_planeta, row): row for row in rows}

        for future in tqdm(as_completed(futures), total=len(rows), unit="planeta", ncols=100):
